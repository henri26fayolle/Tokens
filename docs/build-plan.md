...
- **P2 leaderboard ✅ (2026-07-04)** — public `GET /v1/leaderboard?board=season|lifetime`, web `/ranks`. Ranked by seasonal XP (default) / lifetime, ties by createdAt asc; only XP>0 shown; streaks displayed never ranked; every viewer gets their own position even outside top 50; season #1 = Meijin (名人). No migration (seasonXp existed since M0). Tab bar now complete: Feed·Ranks·Post·Stats·Connect.
- **P2 follows ✅ (2026-07-04)** — directed follows (migration 0006), NO XP either way (test-enforced), self-follow blocked, follow push. `GET /v1/feed?scope=following` (authed) + global Discover feed; profile carries followerCount/followingCount/iFollow/isMe. Web: profile Follow button + counts, feed Following/Discover tabs (signed-in default Following).
- **P2 gateway-prefilled posts ✅ (2026-07-04)** — the brief's <30s posting. Detected moments (deep-session/marathon/new-model, written since M5) now surface as one-tap drafts: home "Share this session?" card → composer prefilled from `?moment=`, verified chips auto-attached. `GET /v1/me/moments/suggestions` (anti-join on posts.momentId, not dismissed, last 7d) + `POST /v1/me/moments/:id/dismiss`. Migration 0007 (moments.dismissedAt).
- **P2 next**: kudos push notifications, URL previews/images, moderation beyond soft-delete (report button before any open signup), invite mechanics for the alpha. Bigger bets (need users first, per user's own roadmap): peer-to-peer review (reputation currency; ~20+ active users), sponsored design contests (Phase 3; needs an audience). Ops (not code): startup credits (Vercel for Startups, Railway; NOT Supabase — we're on Railway PG). Deferred leaderboard extras: "you've been overtaken" push, opt-out-of-ranking flag (before Phase 3 enterprise), season-end Meijin ceremony (season ends Sept 30 → permanent achievement).
