...
- **P2 next**: kudos push notifications, post-from-moment UI (composer prefilled from a detected moment), URL previews/images, moderation beyond soft-delete (report button before any open signup), invite mechanics for the alpha. Deferred leaderboard extras (post-alpha): "you've been overtaken" push, opt-out-of-ranking flag (needed before Phase 3 enterprise), season-end Meijin ceremony (first season ends Sept 30 → permanent achievement).
